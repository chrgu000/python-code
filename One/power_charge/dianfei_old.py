...
class MyFrame(wx.Frame):
    ...
    def __init__(self):
        ...
        self.day_text = wx.TextCtrl(panel, -1,  "75")
    ...
    def btn_ydSaveEVT(self, event):
        data_list = self.getValue()
        self.mf.fWrite(data_list)
        dianfei_list = self.mf.fRead()
        self.text_show.Clear()  # 清除文本显示区内容
        msg_list = ["日期：", "506#余电：", "507#余电：", "508#余电：", "509#余电：",
                    "506#充值：", "507#充值：", "508#充值：", "509#充值："]
        for x in range(len(data_list)):
            if x == (0 or 5 or 9) :
                self.text_show.AppendText("==================\n")
            self.text_show.AppendText(msg_list[x]+str(data_list[x])+'\n')
    ...
    def btn_countEVT(self, event):
        ...
        # int 取整
        int_day = int(self.day_text.GetValue())
        int_506 = int(self.text_506.GetValue())
        int_507 = int(self.text_507.GetValue())
        int_508 = int(self.text_508.GetValue())
        int_509 = int(self.text_509.GetValue())

        result_506 = round(int((dl[1]+dl[5]-int_506)/days*int_day)-int_506, -2)
        result_507 = round(int(((dl[2]+dl[6]-int_507)/days*int_day)-int_507), -2)
        result_508 = round(int(((dl[3]+dl[7]-int_508)/days*int_day)-int_508), -2)
        result_509 = round(int(((dl[4]+dl[8]-int_509)/days*int_day)-int_509), -2)
        # 计算充值需要的金额，单价为1.3元/度
